...
public function withdrawMoney(Request $request)
{
    ...
    if (!$payment_gateway || !$user_id) {
        return comman_message_response('Invalid request data', 422);
    }
    ...
    if (($data['payment_method'] ?? '') === 'bank') {
        ...
        switch ($payment_gateway) {
            ...
            case 'razorpayx':
                ...
                $razorPayX = PaymentGateway::where('type', 'razorPayX')
                    ->where('status', 1)
                    ->first();
                ...
                if (isset($payout['error'])) {
                    ...
                    if (($payout['error']['code'] ?? '') === 'BAD_REQUEST_ERROR') {
                        return comman_message_response(
                            trans('messages.razorpay_message', ['razorpay_message' => $razorpay_message]),
                            406
                        );
                    }
            ...
            case 'stripe':
                ...
                if (isset($response->status) && $response->status == 400) {
                    ...
                    return comman_message_response(
                        trans('messages.stripe_message', ['stripe_message' => $response->code])
                    );
                }
    ...
    // Deduct wallet amount ONLY AFTER SUCCESS / FAILED ENTRY CREATED
    ...
    return comman_message_response(__('messages.money_transfer'));
}
